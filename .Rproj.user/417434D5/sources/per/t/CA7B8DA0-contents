...
##Complete Analysis:
#Most popular mode of transport nationally
...
A1 <- ggplot(Q1, aes(x="", y=counts/sum(counts), fill=modevarlabel)) + geom_bar(stat="identity", width=0.4) +
...
A1a = ggplot(Q1a, aes(x="", y=count/sum(count), fill=pvtnat)) + geom_bar(stat="identity", width=0.4) +
...
A1graph <- ggarrange(A1,A1a, ncol =2, nrow =1) 
print(A1graph)  

#2.My county is Galway County, making reduced records dataset.
...
#3 County as cities in the dataset 
...
#4. Percentage of people leaving outside of 8-9 time period.  
...
#5 People likely to travel more than 45 minutes in my county each morning. 
...
#6 Comparison of Q5 with other counties in same NUTS3 region.
...
#7 Residents experiencing longest commute times, where the longest commute times anything > 90 mins.
comm_grt90 <- survey %>% 
...
#8 Wherever Mode-Car_Passenger was 0, it was understood that only "car drivers" were commuting to work.
...
#9 County specific commuting behaviour in terms of modes.
...
#Areas having lesser number of people travelling by private means on an average.
...
#Areas having lesser number of people travelling by public means on an average.
...
#Areas where on an average lesser number of people leaving house outside 8-9 am.     
...
#Areas where on an average lesser number of people take more than 45 mins to travel. 
x = commdur1[commdur1< mean(commdur1)]
names(x)

#10 Electoral Division on priority for investment in public transport 
